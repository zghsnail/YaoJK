...
- (void)SelectPhotos:(UIButton *)sender {
    ...
    if ([self.ImagesSelectedArray[_pageIndex] isEqualToString:@"0"]) {
        [sender setImage:[UIImage imageNamed:@"2-3-0"] forState:UIControlStateNormal];
        [self.ImagesSelectedArray replaceObjectAtIndex:_pageIndex withObject:@"1"];
        [self.selectedArray addObject:self.assets[_pageIndex]];
    }else {
        [sender setImage:[UIImage imageNamed:@"2-3-1"] forState:UIControlStateNormal];
...
- (NSMutableArray<UIImage *> *)selectImages {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    if (collectionView.tag == 1000) {
        ...
        if (indexPath.row <self.assets.count) {
            ...
            [cell makeImageCell:nil takePhotos:@"2-4-0"];
        }
        cell.delegate = self;
        return cell;
...
- (void)imageIsSelect {
    if ([self.ImagesSelectedArray[_pageIndex] isEqualToString:@"1"]) {
        [_okButton setImage:[[UIImage imageNamed:@"2-3-0"] original] forState:UIControlStateNormal];
        if (![self.selectedArray containsObject:self.assets[_pageIndex]]) {
             [self.selectedArray addObject:self.assets[_pageIndex]];
        }
     
    }else {
        [_okButton setImage:[[UIImage imageNamed:@"2-3-1"]original ]forState:UIControlStateNormal];
        if ([self.selectedArray containsObject:self.assets[_pageIndex]]) {
             [self.selectedArray removeObject:self.assets[_pageIndex]];
        }
