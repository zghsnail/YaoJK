...
+(XLPaymentLoadingHUD*)showIn:(UIView*)view layoutWithFrame:(BOOL)layo color:(UIColor *)color{
    [self hideIn:view];
    XLPaymentLoadingHUD *hud = [[XLPaymentLoadingHUD alloc] initWithFrame:view.bounds layoutWithFrame:layo color:color];
    [hud start];
    [view addSubview:hud];
    return hud;
}
...
- (instancetype)initWithFrame:(CGRect)frame layoutWithFrame:(BOOL)layo color:(UIColor *)color{
    if (self = [super initWithFrame:frame]) {
        layoutWithFrame = layo;
        circleColor = color;
        [self buildUI];
    }
    return self;
...
-(void)buildUI{
    ...
    if (layoutWithFrame) {
        _animationLayer.bounds = CGRectMake(0, 0, self.frame.size.width, self.frame.size.height);
    }else{
    ...
    _animationLayer.strokeColor = circleColor.CGColor;
